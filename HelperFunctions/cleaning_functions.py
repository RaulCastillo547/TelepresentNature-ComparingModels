...
def basic_clean_deer(url_dest = 'CleanCSV\\Deer\\'):    
    # Get Deer Data
    raw_deer_data = pd.read_csv(r'RawCSV\EuroDeer_ Roe deer in Italy 2005-2008.csv')
...
def interpolation_clean_deer(url_dest = 'CleanCSV\\Deer\\'):    
    # Get Deer Data
    raw_deer_data = pd.read_csv(r'RawCSV\EuroDeer_ Roe deer in Italy 2005-2008.csv')
    raw_deer_data['timestamp'] = pd.to_datetime(raw_deer_data['timestamp'])
    raw_deer_data.set_index('timestamp', inplace=True)
    ...
    for local_id in list(raw_deer_data['tag-local-identifier'].unique()):
        ...
        one_deer_data = raw_deer_data[raw_deer_data['tag-local-identifier'] == local_id].copy(deep=True)
        one_deer_data.drop(columns=['tag-local-identifier'], inplace=True)
        ...
        one_deer_data.to_csv(f"{url_dest}{local_id}_interpolated.csv", index=True, index_label='timestamp')

def basic_clean_moose(url_dest = 'CleanCSV\\Moose\\'):
    # Retrieve Data
    raw_moose_data = pd.read_csv(r'RawCSV\ABoVE_ Boutin Alberta Moose.csv')
    ...
    for moose_tag in raw_moose_data['tag-local-identifier'].unique():
        ...
        # Because there are many mooses, we can select for the mooses with the longest timelines
        ...
        specific_moose.to_csv(f'{url_dest}{moose_tag}.csv', index=True, index_label='timestamp')

def interpolation_clean_moose(url_dest = 'CleanCSV\\Moose\\'):
    # Retrieve Data
    raw_moose_data = pd.read_csv(r'RawCSV\ABoVE_ Boutin Alberta Moose.csv')
    raw_moose_data['timestamp'] = pd.DatetimeIndex(raw_moose_data['timestamp'])
